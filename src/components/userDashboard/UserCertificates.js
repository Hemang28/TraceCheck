    ...
    const certificates = [
        {
            ...
            time: '2022-2023',
            accessLog: [
              { name: 'User 1', walletAddress: '0x1234567890abcdef' },
              { name: 'User 2', walletAddress: '0xabcdef1234567890' },
            ],
          },
        {
            id: 2,
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="col-12 d-flex justify-content-end mb-4">
          <button className="btn btn-primary" onClick={handleAddCertificateClick}>+ Add Certificate</button>
        </div>
        
        {certificates.map((certificate) => (
          <div key={certificate.id} className="col-12 col-md-6 col-lg-4 mb-4">
            <div className="card card-container">
              <img src={certificate.image} className="card-img-top user-certificate-card-image" alt="Certificate" />
              <div className="card-body">
                <h5 className="card-title">{certificate.title}</h5>
                <span className={`badge ${getBadgeColor(certificate.status)}`}>{certificate.status}</span>
                <p className="card-text mt-2">Upload Date: {certificate.uploadDate}</p>
